...
async function processSitemapFile(sitemapUrl: string, staticDir: string): Promise<string[]> {
	...
	if (sitemapContent.includes('<sitemapindex')) {
		...
		let match;
		const childSitemapUrls: string[] = []; // <<< CORREÇÃO DE DECLARAÇÃO
 ...
 */
function removeImageTags(xmlContent: string): string {
	...
	cleanedContent = cleanedContent.replace(
		/ xmlns:image="http:\/\/www\.google\.com\/schemas\/sitemap-image\/1\.1"/g,
		''
	);
