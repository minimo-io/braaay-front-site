<script>
	...
	$effect(() => {
		...
		// Clean up previous observer before setting up a new one
		if (observer) {
			if (originalButton) {
				observer.unobserve(originalButton);
			}
			observer.disconnect();
		}
	...
	// Derived state: does the current route need extra padding?
	$effect(() => {
		const routesWithFixedBottomButton = ['/produto/[productSlug]', '/product/[productSlug]'];
		const isProductPage = routesWithFixedBottomButton.includes(page.route?.id || '');
		needsExtraBottomPadding = isProductPage && showFixedButton;
	});
...
{#if !drawerState.active}
	<button
		...
		${needsExtraBottomPadding ? 'bottom-20' : 'bottom-5'} md:bottom-6
