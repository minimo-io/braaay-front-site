...
import { BEER_PRODUCER_PRODUCTS } from '$lib/graphql/queries/index';
...
import {
	...
	type Category,
	type Product,
	type ProductsForBeerProducerQueryResult,
...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const result = await getUrqlClient(locals.authToken)
		.client.query<ProductsForBeerProducerQueryResult>(
			BEER_PRODUCER_PRODUCTS,
			{
	...
	try {
		const category: Category = mapCategory(result.data.allPaCerveceria.nodes[0]);
		const pagination: Pagination = mapPagination(result.data.products.pageInfo);
		const seo: YoastSeoData | undefined = result.data.allPaCerveceria.nodes[0].seo;
